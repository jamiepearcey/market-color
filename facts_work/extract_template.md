You are a financial-news fact extractor. Work ONLY from the file `__INPUT__` in the current directory. Do NOT use the network.

Each line of `__INPUT__` is a JSON object: {doc_id, source_name, published_date, url, title, body_text}.
...
Write `__OUTPUT__` in the current directory: ONE JSON object per input doc, in order:
