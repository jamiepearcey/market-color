...
pub const PROMPT_VERSION: &str = "eg-rich-v1";
...
Extract ONE JSON object (only what the article asserts; do not speculate). Every
causal edge, sensitivity, sentiment, event and proposition MUST cite the chunk it
comes from (evidence_chunk = the [i]) and quote the exact supporting words verbatim.

{{
  "doc_type": "news|analysis|opinion|data_release|corporate_statement|non_financial",
  "sentiment_overall": <-1..1 or null>,
  "entities": [{{"name":"canonical (no tickers/Inc.)","type":"company|bank|central_bank|sovereign|regulator|commodity|currency|equity_index|rate_or_bond|sector|person|exchange|economic_indicator|other","identifier":"ticker/ISO if inferable or null","sector":null,"country":null,"role":"subject|driver|counterparty|mentioned"}}],
  ...
  "causal_edges": [{{"cause":"entities.name","effect":"entities.name","mechanism":"monetary_policy|rate_decision|supply_shock|demand_change|earnings|guidance|mergers_acquisitions|default|rating_action|regulation|geopolitics|fund_flows|data_surprise|contagion|other","effect_direction":"up|down|widen|tighten|volatile|unchanged","magnitude_value":null,"magnitude_unit":"pct|bps|usd|null","modality":"happened|ongoing|forecast|hypothetical|denied","attribution_source":"reporter|named_analyst|company|official|market_consensus|market_implied","lag":"immediate|days|longer","confidence":"strong|tentative","evidence_chunk":0,"quote":"verbatim contiguous span"}}],
  "sensitivities": [{{"asset":"entities.name","factor":"e.g. oil|rates|usd|credit spread|<name>","factor_type":"rates|credit_spread|oil|commodity|usd|fx|equity_beta|inflation|specific|other","sign":1,"magnitude_qual":"high|medium|low","magnitude_value":null,"magnitude_unit":"beta|years|pct_of_cost|null","basis":"fundamental|stated_beta|historical","evidence_chunk":0,"quote":"verbatim"}}],
  ...
  "propositions": [{{"text":"resolvable forward statement","subject":"entities.name or null","resolution_date":"ISO or null","resolution_criteria":"objective settle condition","probability":null,"source":"reporter|named_analyst|market_implied","source_instrument":"fed_funds_future|cds|option|null","contract_hint":null,"evidence_chunk":0,"quote":"verbatim"}}],
  "figures": [{{"entity":"entities.name or null","kind":"price|level|pct|bps|volume|estimate","value":0.0,"unit":null,"evidence_chunk":0,"quote":"verbatim"}}]
}}

Rules: entities 3-8; cause/effect/asset/target/subject MUST be an entities.name; a causal_edge's cause and effect MUST be DIFFERENT entities (never the same one -- if a story is one firm's own news, the cause is the driver/event, not the firm itself); use vocab values exactly; empty arrays where nothing applies; quote copied verbatim from the cited chunk."#,
...
fn lenient_extraction(v: Value) -> DocExtraction {
