...
freq=collections.Counter(); comp_q=collections.defaultdict(list); ev_q=collections.defaultdict(list); ev_named=collections.defaultdict(lambda: collections.defaultdict(int)); ev_docs=collections.defaultdict(set)
for l in open(G/"lake/causal_event_edge.jsonl"):
    ...
    if q and len(comp_q[sym[e]])<50: comp_q[sym[e]].append((q+". "+head.get(did,""))[:200])
...
for (m,key) in evs:
    ...
    for s in uni:
        ...
        E.append(float(ev[(m,key)]@cv[s])); TR.append(cw(R2[s],bp,preW)); CO.append(cw(R2[s],bc,conW)); FW.append(cw(R2[s],bf,fwdW))
...
print(f"\n=== EVENT-LEVEL inference (per-event IC, mean/t/bootstrap across events; T2 broadened non-mention) ===")
