...
export const TreeProvider = ({
  ...
  const handleSelection = useCallback(
    (nodeId: string, ctrlKey = false) => {
      ...
      if (isControlled) onSelectionChange?.(nextSelection);
...
export const TreeNode = ({
  ...
  if (level > 0 && parentPath.length < level - 1) {
    while (currentPath.length < level - 1) currentPath.push(false);
  }
...
export const TreeNodeTrigger = ({
  ...
  return (
    <motion.div
      className={cn(
        "group relative mx-1 flex cursor-pointer items-center rounded-md px-3 py-2 transition-all duration-200 hover:bg-accent/50",
